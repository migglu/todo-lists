...
class AuthneticateTest(TestCase):

    def setUp(self):
        self.backend = PersonaAuthenticationBackend()
        user = User(email="other@user.com")
        user.username = "outheruser"
        user.save()
    ...
    def test_reurns_none_if_status_not_okay(self, mock_post):
    ...
    def test_craetes_new_user_if_necessary_for_valid_assertion(
        self, mock_post
    ):
        mock_post.return_value.json.return_value = {
            "status": "okay",
